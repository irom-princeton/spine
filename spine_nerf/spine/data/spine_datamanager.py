...
# from spine.data.utils.affordance_dataloader import AffordanceDataloader
...
import pdb
@dataclass
class SPINEDataManagerConfig(VanillaDataManagerConfig):
...
class SPINEDataManager(VanillaDataManager):  # pylint: disable=abstract-method
    ...
    def __init__(
        ...
        **kwargs,  # pylint: disable=unused-argument
        ...
        # initialize all cameras as unseen
        self.init_unseen_cameras()
    
        # path to SPINE
        parent_path = Path(__file__).parent.parent.parent.parent.resolve()

        # output directory
        cache_dir = f"{parent_path}/outputs/nerf/{self.config.dataparser.data.name}/dataloader"
    ...
    def init_unseen_cameras(self):
        """Initializes all training cameras as unseen"""
        # initialize all cameras as unseen
    ...
    def next_train(self, step: int) -> Tuple[RayBundle, Dict]:
        ...
        ray_indices = batch["indices"] # (4096, 3)
        ray_bundle = self.train_ray_generator(ray_indices)
        
        # ground-truth RGB
        batch["gt"] = image_batch
        
        # # DINO/VGGT/CLIP embeddings
        # image-space semantics
        batch["img_semantics"] = self.img_semantics_interpolator(ray_indices)# .to(self.device)
        
        # vision-language semantics
        batch["lang_semantics"] = self.lang_semantics_interpolator(ray_indices)
        
        # assume all cameras have the same focal length and image width
        ray_bundle.metadata["fx"] = self.train_dataset.cameras[0].fx.item()
        ray_bundle.metadata["width"] = self.train_dataset.cameras[0].width.item()
        
        # image-space semantics for learning the inverse map
        if self.img_semantics_interpolator.cam_data is not None:
            # select a training camera
            ...
            # TODO: Refactor, e.g., for speed
            # update the "camera semantic embedding" to be image-based, rather than ray-based
            batch["img_semantics"] = (
                self.img_semantics_interpolator.cam_data[image_idx].float().to(self.device),
                batch["img_semantics"][1],
                batch["img_semantics"][2],
            )

            # get camera pose
            camera = self.train_dataset.cameras[image_idx : image_idx + 1].to(self.device)
            batch["camera_to_world"] = camera.camera_to_worlds
