...
def main(cfg):
    ...
    for rad_key in rad_keys:
        eval_cam = spine_eval.rad_field[rad_key]["base"].pipeline.datamanager.eval_dataset.cameras
    
        # TODO: workaround for 3D-OVS scenes (too few or no eval cameras so we use the train dataset)
        if "3d_ovs" in f"{rad_key}".lower():
            eval_cam = spine_eval.rad_field[rad_key]["base"].pipeline.datamanager.train_dataset.cameras
        
        if len(eval_cam) == 1:

            # For NeRFs, insert another camera from the training set (otherwise length(eval_cam) = 1)
            train_cam = spine_eval.rad_field[rad_key]["base"].pipeline.datamanager.train_dataset.cameras

            # query cameras
            cam_to_worlds = torch.concat((
                train_cam.camera_to_worlds[0:1],
                eval_cam.camera_to_worlds,
            ))
        else:
            cam_to_worlds = eval_cam.camera_to_worlds
        ...
        eval_cam = Cameras(
            ...
            cx=eval_cam.cx,
            cy=eval_cam.cy,
            camera_type=eval_cam.camera_type,
            camera_to_worlds=cam_to_worlds,
            
        )
    
        if len(eval_cam) < cfg.evaluation.num_eval_cams:
            # number of interpolation steps for the camera
            num_cam_interp_steps = max(1, np.ceil(cfg.evaluation.num_eval_cams / max(1, len(eval_cam) - 1)).astype(int))

            # interpolate the cameras
            eval_cam = get_interpolated_camera_path(
                cameras=eval_cam,
        ...
        eval_cam = eval_cam[:cfg.evaluation.num_eval_cams]

        eval_cam_all[rad_key] = eval_cam

    # ----------------------------------------------------------------------- #
    # Evaluate: RF Inversion
    # ----------------------------------------------------------------------- #
    rf_inversion_results = spine_eval.evaluate_rf_inversion(
        cameras=eval_cam_all,
    )
    
    # ----------------------------------------------------------------------- #
    # Evaluate: PCA
    # ----------------------------------------------------------------------- #
    rf_pca_results = spine_eval.evaluate_semantics_pca(cameras=eval_cam_all)
