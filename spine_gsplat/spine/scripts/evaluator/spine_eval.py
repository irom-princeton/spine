...
from spine.scripts.utils.inversion_utils import (
    SE3error,
    POI_Detector, 
    vec_to_rot_matrix, 
    execute_PnP_RANSAC,
...
class SpineEval(RadianceFieldEval):
    ...
    def evaluate_semantics_pca(
        ...
        for scene, rf_fd_dict in rad_field.items():
            ...
            for rf_name, rf_fd in rf_fd_dict.items():
                ...
                # save for each scene and rf
                if save_results:
                    print(f":green_book: Saving results to {base_output_path}/stats_{scene}_{rf_name}.json")
    ...
    def _compute_semantics_pca(
        ...
        U_pca_lr, S_pca_lr, V_pca_lr = torch.pca_lowrank(
            ...
            center=True, # default
            niter=20, # default #TODO: mess around
        )
    ...
    def _compute_rf_inversion(
        ...
        with torch.no_grad():
            ...
            cam_pose_dec_output = rad_field.pipeline.model.decode_camera_pose(
    ...
    def _run_rf_fine_inversion(
        ...
        if not isinstance(init_guess, torch.Tensor):
            # init_guess = np.nan_to_num(init_guess, nan=0.0)
        ...
        for p_idx in tqdm(range(len(cameras)), 
                        desc="computing error in pose estimates"):
            
            # if p_idx >= len(cameras) or p_idx >= len(images) or p_idx >= len(init_guess):
            #     print(f"Index {p_idx} is out of bounds!")
            #     continue
        ...
        for ptb_idx in range(len(perturb_init_guess_params["rotation"])):
           ...
           # success flag (i.e., no error thrown)
            ...
            for p_idx in tqdm(range(len(images)), 
                            desc="computing error in pose estimates"):
                if p_idx >= len(cameras) or p_idx >= len(images):
                    print(f"Index {p_idx} is out of bounds for baseline!")
                    continue
