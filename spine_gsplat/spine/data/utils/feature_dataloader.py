...
class FeatureDataloader(ABC):
    ...
    def load(self):
        ...
        if cam_data_file_path.exists():
            self.cam_data = torch.from_numpy(np.load(cam_data_file_path)).to(self.device)
            
        # image data
        img_data_file_path = Path(f"{file_path_without_ext}_img_data{file_ext}")
        
        if img_data_file_path.exists():
            # self.img_data = torch.from_numpy(np.load(img_data_file_path)).to(self.device)
            self.img_data = torch.from_numpy(np.load(img_data_file_path, mmap_mode="r")) # Avoid OOM
        
        # augmented data (embeds)
        aug_data_file_path = Path(f"{file_path_without_ext}_aug_data{file_ext}")
        
        if aug_data_file_path.exists():
            self.data_aug_embeds = torch.from_numpy(np.load(aug_data_file_path)).to(self.device)
